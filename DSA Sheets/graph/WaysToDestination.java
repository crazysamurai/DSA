...
//yt:https://www.youtube.com/watch?v=_-0mx0SmYxA&list=PLgUwDviBIf0oE3gA41TKO2H5bHpPd7fzn&index=41
        ...
        while (!pq.isEmpty()) {
            ...
            for (Pair val : adj.get(node)) {
                ...
                if (weight + currTime < time[adjNode]) {
                    ...
                    ways[adjNode] = ways[node];// ways to reach the adjacent node and parent node will be the same if
                                               // adjNode has not been visited before or better time is found
                } else if (weight + currTime == time[adjNode]) {// if a node is reached again with the same time as
                                                                // stored in the distance array then don't add it to the
                                                                // queue again and ways to reach that node will be
